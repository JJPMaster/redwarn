...
-----------------------------------------------------------------------------
Copyright 2020 The RedWarn Development Team, adapted by JJPMaster

The original version of RedWarn was licensed under the Apache License, 
Version 2.0 (the "License"); a copy of the license can be found below:

    http://www.apache.org/licenses/LICENSE-2.0

This modified version of RedWarn is licensed under the GNU General Public
License version 3.
...
RedWarn 16.1 brings further UX improvements and feature additions.
    ...
    /**
     * This varible is defined by the magic word "Build Time: 04/01/2021 18:23:57UTC
Excecuted script: /builds/redwarn/redwarn-web/build.php
User: root@runner-72989761-project-19374445-concurrent-0 on Linux", which the build script will replace with information
    ...
    "buildInfo" : `Build Time: 04/01/2021 18:23:57UTC
Excecuted script: /builds/redwarn/redwarn-web/build.php
User: root@runner-72989761-project-19374445-concurrent-0 on Linux`,
...
function initRW() {
    rw.visuals.init(()=>{
        ...
        rw.info.getConfig(()=> {
            ...
            rw.info.featureRestrictPermissionLevel(/extendedconfirmed|rollback/, ()=>{}, ()=>{rw.userIsNotEC = true;});
...
rw.rulesFunc = {
    "resync": callback => {
        $.getJSON("https://en.uncyclopedia.co/w/index.php?title=Uncyclopedia:RedWarn/Default_Warnings&action=raw&ctype=text/json", rules => {
    ...
    "load": callback => {
        $.getJSON("https://en.uncyclopedia.co/w/index.php?title=User:" + rw.info.getUsername() + "/redwarnRules.json&action=raw&ctype=text/json", rules => {
    ...
    "save": callback => {
        ...
        $.post(rw.wikiAPI, {  // LOCALISATION ISSUE!!
            ...
            "summary": "Updating user Rule Database [[UN:RW|(RW " + rw.version + ")]]", // summary sign here
...
rw.info = { // API
    ...
    "getRollbackToken" : () => {
        ...
        rw.info.featureRestrictPermissionLevel("rollback", ()=>{
...
window.rw = window.rw || {}, window.rw.config = ` + JSON.stringify(rw.config) + "; //</no" + "wiki>"; // generate config text
            $.post(rw.wikiAPI, {  // LOCALISATION ISSUE!!
                ...
                "summary": "Updating user configuration [[UN:RW|(RW " + rw.version + ")]]", // summary sign here
                "text": finalTxt,
                "tags": ((rw.wikiID == "enwiki") ? "RedWarn" : null) // Only add tags if on english wikipedia
            }).done(dt => {
                // We done. Check for errors, then callback appropriately
    ...
    "addWikiTextToUserPage" : (user, text, underDate, summary, blacklist, blacklistToast, callback) => {
        ...
        $.getJSON(rw.wikiAPI + "?action=query&prop=revisions&titles=User_talk:"+user+"&rvslots=*&rvprop=content&formatversion=2&format=json", latestR=>{
            ...
            $.post(rw.wikiAPI, {
                ...
                "summary" : summary + " [[UN:RW|(RW "+ rw.version +")]]", // summary sign here
                "text": finalTxt,
                "tags" : ((rw.wikiID == "enwiki") ? "RedWarn" : null) // Only add tags if on english wikipedia
            }).done(dt => {
                // We done. Check for errors, then callback appropriately
...
rw.rollback = { // Rollback features - this is where the business happens, people!
    ...
    "icons": [ // rev14, icon IDs and everything for current rollback - from left to right - usually loaded from config
...

        {
            ...
            "actionType": "rollback",
            "promptReason": false, // add extra info?
            "summary": "Likely [[UN:(C)|copyright violation]]", // Set summary
            "ruleIndex": "copyright" // used for autowarn
        },

        {
            "enabled": false, // true is a default rollback icon, false can be added via preferences
            "name": "Quick rollback BDH violation",
            "color": "red", // css colour
            "icon": "face",
            "actionType": "rollback",
            "promptReason": false, // add extra info?
            "summary": "Fails [[UN:BDH]]", // Set summary
...

        {
            ...
            "actionType": "rollback",
            "promptReason": false, // add extra info?
            "summary": "Using Uncyclopedia for [[UN:CM|advertising and/or promotion]] is not permitted.", // Set summary
            "ruleIndex": "advert" // used for autowarn
        },

        {
            "enabled": false, // true is a default rollback icon, false can be added via preferences
            "name": "Quick rollback external link spam",
            "color": "red", // css colour
            "icon": "link_off",
            "actionType": "rollback",
            "promptReason": false, // add extra info?
        ...
        {
            "enabled": false, // true is a default rollback icon, false can be added via preferences
            "name": "Quick rollback reliable source",
            "color": "orange", // css colour
            "icon": "history_edu",
            "actionType": "rollback",
            "promptReason": false, // add extra info?
            "summary": "Providing a reliable source", // Set summary
        ...
        {
            "enabled": false, // true is a default rollback icon, false can be added via preferences
            "name": "Quick rollback serious edit",
            "color": "orange", // css colour
            "icon": "child_care",
            "actionType": "rollback",
            "promptReason": false, // add extra info?
            "summary": "Serious edit", // Set summary
...

        {
            ...
            "actionType": "rollback",
            "promptReason": false, // add extra info?
            "summary": "[[UN:SAND|test edits]]", // Set summary
        ...
        rw.info.isLatestRevision(mw.config.get("wgRelevantPageName"), rw.rollback.getRollbackrevID(), (un, crID) => {
            ...
            let pseudoRollbackCallback = () => { // pseudoRollback
                ...
                rw.info.latestRevisionNotByUser(mw.config.get("wgRelevantPageName"), un, (content, summary, rID, pID) => {
                    ...
                    $.post(rw.wikiAPI, {
                        ...
                        "token": mw.user.tokens.get("csrfToken"),
                        "title": mw.config.get("wgRelevantPageName"),
                        "summary": summary + ": " + reason + " [[UN:RW|(RW " + rw.version + ")]]", // summary sign here
                        "undo": crID, // current
            ...
            let rollbackCallback = () => { // using rollback API
                ...
                $.post(rw.wikiAPI, {
                    ...
                    "title": mw.config.get("wgRelevantPageName"),
                    "summary": "Rollback edit(s) by [[Special:Contributions/" + un + "|" + un + "]] ([[User_talk:" + un + "|talk]]): " + reason + " [[UN:RW|(RW " + rw.version + ")]]", // summary sign here
        ...
        $.getJSON(rw.wikiAPI + "?action=query&prop=revisions&titles=" + encodeURIComponent(mw.config.get("wgRelevantPageName")) + "&rvslots=*&rvprop=ids%7Cuser&formatversion=2&format=json", r => {
            ...
            $.getJSON(rw.wikiAPI + "?action=query&prop=revisions&rvprop=user&rvstartid=" + revID + "&rvendid=" + revID + "&titles=" + encodeURI(mw.config.get("wgRelevantPageName")) + "&formatversion=2&rvslots=*&format=json", r => {
                ...
                $.post(rw.wikiAPI, {
                    ...
                    "token": mw.user.tokens.get("csrfToken"),
                    "title": mw.config.get("wgRelevantPageName"),
                    "summary": summary + (reason != null ? ": " + reason : "") + " [[UN:RW|(RW " + rw.version + ")]]", // summary sign here
                    "undo": crID, // current
...
<b>Important:</b> Please be aware that this can take a while to process on longer pages with larger changes, so if the progress bar freezes, please be paitent as RedWarn is still processing in the background.
    ...
    "openAdminReport": (un, doNotShowDialog) => { // Open admin report dialog
        ...
        const aivPage = (rw.debugMenu == null ? "Uncyclopedia:Ban Patrol/Semi-automated" : "User:Ed6767/sandbox");
        ...
        addMessageHandler("AIVreport`*", m => {
            ...
            $.getJSON(rw.wikiAPI + "?action=query&prop=revisions&titles=" + aivPage + "&rvslots=*&rvprop=content&formatversion=2&format=json", latestR => {
                ...
                $.post(rw.wikiAPI, {
                    ...
                    "token": mw.user.tokens.get("csrfToken"),
                    "title": aivPage,
                    "summary": `Reporting [[Special:Contributions/${target}|${target}]] [[UN:RW|(RW ${rw.version})]]`, // summary sign here
                    "text": finalTxt,
                    "tags": ((rw.wikiID == "enwiki") ? "RedWarn" : null) // Only add tags if on english wikipedia
                }).done(dt => {
                    // We done. Check for errors, then callback appropriately
    ...
    "beginUAAReport": (un, htmlOnly) => { // Report to UAA
        ...
        const uaaPage = (rw.debugMenu == null ? "Uncyclopedia:Ban Patrol/Semi-automated" : "User:Ed6767/sandbox"); // set UAA based on debug mode
        ...
        addMessageHandler("UAA`*", m => {
            ...
            $.getJSON(rw.wikiAPI + "?action=query&prop=revisions&titles=" + uaaPage + "&rvslots=*&rvprop=content&formatversion=2&format=json", latestR => {
                ...
                // Let's continue
                // We don't need to do anything special. Just shove our report at the bottom of the page, although, may be advisiable to change this if ARV format changes
                let textToAdd = "*" + "{{user|1=" + target + "}} &ndash; " + reportContent; // DANGER! WIKITEXT (here is fine. be careful w changes.) - if target IP give correct template, else normal
                let finalTxt = revisionWikitext + "\n\n" + textToAdd; // compile final string
                // Now we just submit
                $.post(rw.wikiAPI, {
                    "action": "edit",
    ...
    "openExtendedOptionsDialog": un => {
        ...
        $.getJSON(rw.wikiAPI + "?action=query&meta=userinfo&uiprop=email&format=json", r => {
            ...
            addMessageHandler("TASEmail`*", e => dialogEngine.closeDialog(() => rw.info.sendEmail("Kev", atob(e.split("`")[1]))));

            // Email to OS - THESE ARE LIVE
            addMessageHandler("OSEmail`*", e => dialogEngine.closeDialog(() => rw.info.sendEmail("Kev", atob(e.split("`")[1]))));
                            ...
                            addMessageHandler("reason`*", reasonIn=> {
                                ...
                                $.ajax({
                                    ...
                                    data : {
                                        ...
                                        "wpReason" : comment + " ([[UN:RW|RW "+ rw.version + "]])",
                        ...
                        rw.PendingChangesReview.getPendingChangesUsers((usr, count, users, userCount)=>{
                            ...
                            addMessageHandler("reason`*", reasonIn=> { // ON REASON RECIEVED
                                ...
                                $.ajax({
                                    ...
                                    "fileVersion": "",
                                    "validatedParams": $('input[name ="validatedParams"]').attr("value"),
                                    "wpReason" : revertString+ (comment.length > 0 ? ": "+ comment : "") + " ([[UN:RW|RW "+ rw.version + "]])",
            ...
            $("#rReviewUnAccept").click(()=>{
                addMessageHandler("reason`*", reasonIn=> { // ON REASON RECIEVED 
                    ...
                    $.post("https://en.wikipedia.org/w/index.php?title=Special:RevisionReview&action=submit", {
                        ...
                        "fileVersion": "",
                        "validatedParams": $('input[name ="validatedParams"]').attr("value"),
                        "wpReason" : "Unapproving"+ (comment.length > 0 ? ": "+ comment : "") + " ([[UN:RW|RW "+ rw.version + "]])",
...
rw.multiAct = { // Multi action screen
    ...
    "open" : userObj=>{ // userobj format or just {}
        ...
        rw.info.featureRestrictPermissionLevel("extendedconfirmed", ()=>{
            ...
            // When no perms
            rw.ui.confirmDialog(`
            Sorry, but to prevent misuse you have to be an <a href="https://en.wikipedia.org/wiki/Wikipedia:User_access_levels#Extendedconfirmed" target="_blank">extended confirmed user</a> or rollbacker to use the multiple action tool. Check back later!
            `,
            "OKAY", ()=>dialogEngine.closeDialog(),
            "", ()=>{}, 23);
        });
...
// </nowiki>
        ...
        $.post(rw.wikiAPI, {
            ...
            "token" : mw.user.tokens.get("csrfToken"),
            "title" : packPage,
            "summary" : "Publish Autogenerated QTPack Install [[UN:RW|(RedWarn "+ rw.version +")]]", // summary sign here
            ...
            if (!dt.edit) {
                ...
                rw.info.writeConfig(true, ()=>{ // save config
                    ...
                    $.post(rw.wikiAPI, {
                        ...
                        "summary" : "Publish new pack [[UN:RW|(RedWarn "+ rw.version +")]]", // summary sign here
...
rw.pageProtect = { // Used for [[WP:RFPP]]
     ...
     */
    "rfppPage" : "User talk:Kev/protection requests", // !!! FOR PRODUCTION USE Wikipedia:Requests_for_page_protection ELSE USE User:Ed6767/sandbox/rwTests/rpp !!!
    ...
    "open" : ()=>{ // Open page protection dialog for this page
        ...
        rw.pageProtect.getCurrentPageProtection(protection=>{
            ...
            addMessageHandler("submitRFPP`*", m=>{
                ...
                let text = `== Protection request on [[:${mw.config.get("wgRelevantPageName").replace(/_/g, ' ')}]] ==
...
'''${(requestProtect.name == "no protection" ? "Unprotection" : requestDuration + " " + requestProtect.name)}:''' ${(changeCoreReason == "Other rationale" ? "" : changeCoreReason + `. `)}${changeExtraInfo} `+ rw.sign();
                ...
                $.getJSON(rw.wikiAPI + "?action=query&prop=revisions&titles="+ encodeURIComponent(rw.pageProtect.rfppPage) +"&rvslots=*&rvprop=content&formatversion=2&format=json", latestR=>{
                    ...
                    $.post(rw.wikiAPI, {
                        ...
                        "summary" : `Requesting protection change for [[${mw.config.get("wgRelevantPageName").replace(/_/g, ' ')}]] [[UN:RW|(RW ${rw.version})]]`, // summary sign here
                        "text": finalTxt,
                        "tags" : ((rw.wikiID == "enwiki") ? "RedWarn" : null) // Only add tags if on english wikipedia
                    }).done(dt => {
                        // We done. Check for errors, then callback appropriately
...
rw.campaigns = {
    "load" : ()=>{ // the only function that handles everything
        ...
        $.getJSON("https://en.uncyclopedia.co/w/index.php?title=Uncyclopedia:RedWarn/Campaigns&action=raw&ctype=text/json", c=>{
